...
namespace Asset_Packs.Tilemap_Pack.Assets.Tilemap.Tiles.Rule_Tile.Scripts
{
		...
		public virtual void RuleOnGUI(Rect rect, Vector2Int pos, int neighbor)
		{
			switch (neighbor)
			{
				...
				case RuleTile.TilingRule.Neighbor.This:
					UnityEngine.GUI.DrawTexture(rect, arrows[pos.y * 3 + pos.x]);
					break;
				case RuleTile.TilingRule.Neighbor.NotThis:
					UnityEngine.GUI.DrawTexture(rect, arrows[9]);
					break;
				default:
					var style = new GUIStyle();
					style.alignment = TextAnchor.MiddleCenter;
					style.fontSize = 10;
					UnityEngine.GUI.Label(rect, neighbor.ToString(), style);
					break;
			}
			...
			foreach (var c in allConsts)
			{
				...
				{
					UnityEngine.GUI.Label(rect, new GUIContent("", c.Name));
					break;
				}
			}
		}
        ...
        public override void GetTileData(Vector3Int position, ITilemap tilemap, ref TileData tileData)
		{
			...
			foreach (TilingRule rule in m_TilingRules)
			{
				...
				if (RuleMatches(rule, ref neighboringTiles, ref transform))
				{
                    ...
                    // Converts the tile's rotation matrix to a quaternion to be used by the instantiated Game ObjectDefinition
